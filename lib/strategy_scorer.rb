...
module RoutingEngine
  ...
  class StrategyScorer
    ...
    # Комиссия провайдера — прямые деньги, а не абстрактная цель. В hard-constraints
    # маржа участвует только как порог «не хуже соглашения»; здесь она влияет на
    # предпочтение: среди допущенных выгоднее тот, кто оставляет площадке больше.
    # Значение — доля маржи мерчанта, остающаяся после комиссии провайдера,
    # то есть 0..1 и сравнимо по масштабу с конверсией.
    factor(:margin_advantage, 'сколько маржи остаётся площадке') do |p, _op, _s|
      merchant = p.merchant_margin_pct.to_f
      next 0.0 unless merchant.positive?

      ((merchant - p.provider_margin_pct.to_f) / merchant).clamp(0.0, 1.0)
    end
