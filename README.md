...
**Живой стенд:** <https://a1b9f4d2.51-250-106-42.sslip.io/> — движок считает на машине,
не в браузере; там же [разбор решений](https://a1b9f4d2.51-250-106-42.sslip.io/review).
Открыть с телефона:

<img src="docs/stand-qr.png" alt="QR-код на живой стенд" width="150">
...
[Своя стратегия](#своя-стратегия-что-уносишь-со-стенда) ·
...
```bash
ruby bin/run.rb data/operations_queue_test.json data/providers.json \
                data/operations_history.csv config/strategy.yml \
...
## Своя стратегия: что уносишь со стенда

Политика роутинга здесь — конфигурация, а не код. Поэтому главный вопрос к стенду
не «красиво ли крутятся ползунки», а **что остаётся на руках после того, как
стратегию подобрали**. Остаются три файла, каждый со своей ролью:

| Файл | Что это | Куда дальше |
|---|---|---|
| `strategy.yml` | сама политика: веса, пороги, бизнес-поля | в `config/`, в git, в CI |
| `routing_decisions.json` | решения прогона с полным следом попыток | формат сдачи |
| `routing_report.json` | аналитика прогона: доли, стоимость, отсев, загрузка | в отчётность |

Первый — главный. Два других показывают, что политика даёт на конкретной очереди.

### Почему это не декорация

Выгруженный `strategy.yml` — не отчёт «как было настроено», а рабочий конфиг того
же движка. Подаёшь его четвёртым аргументом:

```bash
ruby bin/run.rb data/operations_queue_test.json data/providers.json \
                data/operations_history.csv strategy.yml \
                routing_decisions.json routing_report.json
```

и получаешь **побайтово те же решения**, что стенд показал на экране. Проверено не
на игрушечной десятке, а на закрытой очереди организаторов: 90 заявок, все 90
решений совпали при изменённых весах и другом seed.

Практический смысл простой. Продуктовый менеджер подбирает политику руками, видя
последствия каждого движения ползунка: как поедут доли, сколько добавится комиссии,
кто упрётся в лимит. Результат — файл, который отправляется в репозиторий и
исполняется в бою тем же кодом. Между «подобрали» и «работает» не остаётся
переписывания на другом языке, а значит и расхождения между тем, что показали, и
тем, что поехало.

### Новая выборка данных

Очередь не зашита нигде. `bin/run.rb` берёт её первым аргументом. Стенд подхватывает
все файлы `data/operations_queue*.json` и даёт выбрать в интерфейсе, а кнопкой
«Загрузить свою очередь» можно скормить произвольный JSON прямо из браузера —
он уходит телом запроса и нигде не сохраняется.

Присланные данные проверяются на входе, чтобы ошибка в файле выглядела как ошибка
в файле, а не как падение стенда: не массив, пустая очередь, заявка без
`operation_id` или `amount` — всё это `400` с внятным текстом. Длина ограничена
2000 заявками, тело запроса — мегабайтом.
...
Отдельная мелочь, стоившая дороже, чем кажется: браузеры всё чаще сами пробуют
`https://`, а TLS у голого порта нет. Раньше рукопожатие читалось как строка
запроса, и сервер молчал до таймаута — для человека это неотличимо от упавшего
сайта. Теперь соединение, начинающееся с байта `0x16`, обрывается сразу.
Живой адрес выше идёт через отдельный сайт в reverse proxy с сертификатом
Let's Encrypt, так что там эта развилка не возникает вовсе.
