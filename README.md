...
Очередь не зашита: `bin/run.rb` берёт её первым аргументом, а стенд подхватывает
все файлы `data/operations_queue*.json` и даёт выбрать в интерфейсе. Прислали новую
выборку — кладёшь файл рядом и работаешь с ней; движок, веса и выгрузка те же.
