...
**Переключение стратегий.** Веса меняются — распределение пересчитывается целиком,
кода никто не трогает.
...
Сам стенд лежит в ветке `main`: здесь оставлен только движок, чтобы в дереве
репозитория не было ничего, кроме Ruby.
