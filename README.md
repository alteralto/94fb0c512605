...
Запись работы стенда: [`docs/stand-cast.mp4`](docs/stand-cast.mp4) — переключение
стратегий, выключение провайдера, ручная правка веса и разбор одной заявки.
Сам стенд лежит в ветке `main`: в этой ветке оставлен только движок, чтобы
в репозитории не было ничего, кроме Ruby.

**Сценарий «провайдер выключен».** payflow отключён, и его целевые 35%
перераспределяются между доступными — vipay получает 61.5%, quickpay 38.5%.
Роутер сверяется уже с эффективной целью, а не с недостижимой записью в
`providers.json`.

![Перераспределение доли выключенного провайдера](docs/stand-provider-offline.png)
...
| `lib/pipeline.rb` | сборка прогона целиком |
...
замером на 40 сидах: при 0.5 эффективная ставка падает с 0.926% до 0.907%, но
среднее отклонение от целей растёт с 23 до 36 п.п. Вывод, который стоит сказать
вслух: при заданных целевых долях экономия на комиссии почти исчерпана — план
жёстче, чем разброс тарифов, и стоит он около 40 ₽ на десяти заявках. Отчёт
считает это в рублях, чтобы решение принимали не на ощущениях.
